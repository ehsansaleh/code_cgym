...
class StandupRewardGiver {
  public:
    StandupRewardGiver();                    // This is the constructor
    //~StandupRewardGiver();                 // This is the destructor
...
/////////////////////////////////////////////////////
/////// NLRRewardGiver Class Definitions ////////
/////////////////////////////////////////////////////

class NLRRewardGiver {
  public:
    NLRRewardGiver();                        // This is the constructor
    ~NLRRewardGiver();                       // This is the destructor
    void reset(double obs_current[obs_dim],  // This should be current, since we'll be applying a
                                             // reward delay (i.e., the same as observation delay)
               bool is_mj_stable,
               SimInterface* simintf);               // re-initialize the reward giver state
  ...
  private:
    SimInterface* simif;
    int nlrbuffidx;
    double theta_time_sig[nlr_buflen];
    double sp2ac_sum, sp2ac_bad_sum, freq_ampsq, dc_diff, ac_size_err;
    #if FFTLIB == NOFFT
      // nothing needed
    #elif FFTLIB == FFTWLIB
      fftw_plan nlr_fftw_plan;
      fftw_complex theta_freq_sig[fftout_len];
      double theta_freqabssq_sig[fftout_len];
    #else
...
class SimInterface {
  public:
    ...
    double state[2]; // theta and thetadot
